...
app.use(session({
  ...
  secret: '암호화에 쓸 비번',
  // 유저가 서버로 요청할 때 마다 session 갱신 여부 = false
  resave : false,
  // 로그인 안 했을때 session 생성 여부
  saveUninitialized : false,
  // session document 유효기간 변경
...
// Javascript는 오래 걸리는 코드가 처리되는 것을 기다리지 않고 바로 다음줄을 실행한다.
  ...
  upload.single('img1')(request, response, async (err) => {
    ...
    try {
      ...
      if(request.body.title === ''){
        response.send('제목을 입력 하세요.');
      } else if (request.body.content === ''){
        response.send('내용을 입력 하세요.');
      } else {
...
app.get('/detail/:id', async(request, response) => {
  try {
    ...
    if(result == null) {
      response.status(404).send('url 입력 에러');
    }
...
function checkInput(request, response, next){
  if(request.body.username === '' | request.body.password === '') {
    response.send('아이디 또는 비밀번호를 입력하세요.')
  } else {
    next();
  }
}
...
app.post('/register', async(request, response) => {
  ...
  db.collection('user').insertOne({ 
    ...
    // 비밀번호는 hasing(문자 -> 랜덤문자 변환)해서 저장하는 게 좋다.
